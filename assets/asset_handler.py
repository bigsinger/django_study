...
class NewAsset(object):
    ...
    def add_to_new_assets_zone(self):
        manufacturer_obj, _ = models.Manufacturer.objects.get_or_create(name=self.data.get('manufacturer'))
        defaults = {
        ...
        # 为什么不能这样用？参考：https://stackoverflow.com/questions/31418714/update-or-create-wont-respect-unique-key
        # obj, created = models.NewAssetToApprove.objects.update_or_create(sn=self.data['sn'], manufacturer=manufacturer_obj, defaults=defaults)
        obj, created = models.NewAssetToApprove.objects.update_or_create(sn=self.data['sn'], defaults=defaults)
        print(created)
...
class ApproveAsset:
    ...
    def __init__(self, request, asset_id):
        self.request = request
        self.new_asset = models.NewAssetToApprove.objects.get(id=asset_id)
    ...
    def _create_asset(self):
        ...
        # asset_obj = models.Asset.objects.get(sn=self.new_asset.sn)
        asset, created = models.Asset.objects.update_or_create(asset_type=self.new_asset.asset_type,
                                            name="%s: %s" % (self.new_asset.asset_type, self.new_asset.sn),
                                            sn=self.new_asset.sn,
                                            approved_by=self.request.user,
                                            )
        print('审批资产入库，是否新建：' + str(created))
        return asset
    ...
    def _create_RAM(self, asset):
        ...
        for ram_dict in ram_list:
            ...
            manu_name = ram_dict.get('manufacturer')
            if manu_name:
                manufacturer_obj, _ = models.Manufacturer.objects.get_or_create(name=manu_name)
                ram.manufacturer = manufacturer_obj
    ...
    def _create_disk(self, asset):
        ...
        for disk_dict in disk_list:
            ...
            manu_name = disk_dict.get('manufacturer')
            if manu_name:
                manufacturer_obj, _ = models.Manufacturer.objects.get_or_create(name=manu_name)
                disk.manufacturer = manufacturer_obj
    ...
    def _update_RAM(self):
        ...
        if new_rams_dict:
            for key in new_rams_dict:
                ...
                manufacturer_obj, _ = models.Manufacturer.objects.get_or_create(name=new_rams_dict[key].get('manufacturer'))
                models.RAM.objects.update_or_create(asset=self.asset, slot=key, manufacturer=manufacturer_obj, defaults=defaults)
    ...
    def _update_disk(self):
        ...
        if new_disks_dict:
            for key in new_disks_dict:
                ...
                manufacturer_obj, _ = models.Manufacturer.objects.get_or_create(name=new_disks_dict[key].get('manufacturer'))
                obj, created = models.Disk.objects.update_or_create(asset=self.asset, sn=key, manufacturer=manufacturer_obj, defaults=defaults)
                print(created)
