...
class CPU(models.Model):
    ...
    asset = models.OneToOneField('Asset', verbose_name='关联资产', on_delete=models.CASCADE)  # 设备上的cpu肯定都是一样的，所以不需要建立多个cpu数据，一条就可以，因此使用一对一。
...
class RAM(models.Model):
    ...
    capacity = models.PositiveIntegerField('内存大小（GB）', blank=True, null=True)
...
class BusinessUnit(models.Model):
    ...
    parent_unit = models.ForeignKey('self', verbose_name='所属部门', blank=True, null=True, related_name='parent_level', on_delete=models.CASCADE)
...
class EventLog(models.Model):
    ...
    new_asset = models.ForeignKey('NewAssetToApprove', blank=True, null=True, on_delete=models.SET_NULL)  # 当资产审批失败时有这项数据
...
class NewAssetToApprove(models.Model):
    """新资产待审批区"""
